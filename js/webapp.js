...
app.controller('index_ctrl',function($scope,learningCommon,$cookieStore,$localStorage,$sessionStorage,$filter,$http,$sce){
    ...
    {
        try{
            $http.get($scope.commonobj.shama_hostname_url+"shama_local_file.php",{server_working:1}).then(function(r){
               if(r.status == 200)
               {
...
app.controller('lesson_ctrl',function($scope,learningCommon,$sce,$cookieStore,$route, $routeParams,$localStorage,$sessionStorage,$filter,$window,$location,$interval,$http){
    ...
    $scope.localServerStatus = null;

    $scope.load_local_file =function(file)
    {
        try{
            $http.get($scope.commonobj.shama_hostname_url+"shama_local_file.php",{"filename":file}).then(function(r){
               if(r.status == 200)
               {
                   $scope.localServerStatus = true;
                    return true;
               }
               else{
                   $scope.localServerStatus = false;
               }
        ...
        try{
...

            {
                ...
                if($scope.commonobj.localStorage.shama_host)
                {
                    ...
                    $scope.load_local_file(file_name);
                    
                    $scope.$watch(function(){
                        return $scope.localServerStatus;
                    },function(newValue,oldValue){
                       if(newValue != null && $scope.localServerStatus)
                       {
                           file_location = file_name;
                            $scope.videoPlay(file_location);
                            $("#myModal").modal();
                       }
                       else if($scope.localServerStatus == false){
                   
                            $scope.videoPlay($scope.lessonobj.currentlesson.content);
                            $("#myModal").modal();
                       }
                    });
    ...
    $scope.videoPlay = function(file_location)
    {
        try{
