 ...
 */
 class User extends MY_Model{
    ...
    */
     ...
     * Student signup
     */
  public function StudentInfo($id,$sname, $slastname,$saddress ,$shunit ,$scity ,$sprovice ,$spcode ,$sphone ,$sdob ,$sdateav ,$snic,$smthrlng,$saddlang ,$sgrade ,$sgrade1 ,$sfname ,$sfnic ,$sfpro ,$sfyear ,$sfcompany ,$sfcyears ,$sfmincome ,$sfwaddress ,$sfmincome2 ,$srassitance ,$scir ,$spreviousshool,$spradress,$sfrom,$sto,$spreviousshool2,$spradress2,$sfrom2,$sto2,$spreviousshool3,$spradress3,$sfrom3,$sto3,$srfulname,$srrelationship,$srcmpny,$srphone,$sradress,$srfulname2,$srrelationship2,$srcmpny2,$srphone2,$sradress2,$srfulname3,$srrelationship3,$srcmpny3,$srphone3,$sradress3,$semester,$ssignature,$ssubmitdate)
  {
  	try{
		$value = FALSE;
		$today = date("Y-m-d");
...
*/
  ...
  /*
    ...
    function finduseimage($id)
    {
  	...
  	public function TeacherInfo($id, $tfname, $tsname, $tgndr, $tnic, $temail, $tphone, $tnpswrd, $tprhmadrss, $tsechmadress, $tprovince, $tcity, $tzipcode, $tMaster){
		$value = FALSE;
		$today = date("Y-m-d");
		$year = date("Y");
		$locations = $this->session->userdata('locations');
		if($id != null)
		{
			...
			$userarray = array(
				...
				'profile_image'=>($this->session->userdata('laststudentimgid') != '' ? IMAGE_LINK_PATH.'profile/'.$this->session->userdata('laststudentimgid') :$is_user_already_exist[0]->profile_image )
		...
		if($id == null && $this->getDublicate($tnic) != true){
			...
			$userarray = array(
				...
				'nic'=>trim($tnic),
				'slug'=>time(),
				'profile_image'=>($this->session->userdata('laststudentimgid') != '' ? IMAGE_LINK_PATH.'profile/'.$this->session->userdata('laststudentimgid') : '' )
