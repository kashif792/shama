...
<script type="text/javascript">
    ...
    app.controller('CalendarCtrl', function ($scope,$compile,$http,$filter,$modal,$rootScope) {
        ...
        $scope.defaultdate = function()
        {
            try{
                $scope.holidays.date = {
                    ...
                    endDate: moment().tz('Asia/Karachi')
                };
        ...
        $scope.save = function(holiday)
        {
            try{
                if(holiday.title)
                {
                    ...
                    httppostrequest('saveholiday', $scope.holidays).then(function(response){
                        if(response != null)
                        {
                            $scope.updateSemesterLessonPlan();
                            getHolidays();
                            $scope.updateObject();
        ...
        $scope.updateSemesterLessonPlan = function()
        {
            try{
                httppostrequest('updatelessonplan',$scope.holidays).then(function(response){
        ...
        $scope.setModalValues = function(calEvent)
        {
            ...
            if(calEvent.serial)
            {
                ...
                if(is_event_found.length > 0)
                {
                    ...
                    console.log('setModalValues', $scope.holidays);
                    return;
                }
            }
        }
