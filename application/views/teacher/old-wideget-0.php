...
<script>
    ...
    app.controller('progress_ctrl', function($scope, $window, $http, $document, $timeout,$interval,$compile){
        ...
        $scope.chnagefilter = function()
        {
            ...
            // console.log($scope.filterobj.section.id);
            // console.log($scope.sbj_id);
            ...
            getLessonPlanList($scope.sbj_id,$scope.filterobj.section.id,$scope.semester_id,$scope.session_id,$scope.filterobj.class.id);
        }
            ...
            try{
                httprequest(urlist.progressreport,({})).then(function(response){
                    if(response != null && response.length > 0)
                    {
                        ...
                        {
                            $scope.sbj_id = response[0].sectionlist[0].subjectlist[0].sbid;
                            $scope.semester_id = response[0].semsterid;
                            $scope.session_id = response[0].sessionid;
                            getLessonPlanList(response[0].sectionlist[0].subjectlist[0].sbid,response[0].sectionlist[0].sid,response[0].semsterid,response[0].sessionid,response[0].classid)
                            $scope.reloadcontent();
                            $scope.reloadresult();
                        }
