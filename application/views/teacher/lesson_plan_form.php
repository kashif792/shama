...
<div class="col-sm-10 dplan-widget plan-widget modified-header"  ng-controller="lesson_plan_controler">
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="DeleteLesson" class="btn btn-default " value="save">Yes</button>
        ...
        <div class="row">
                        ...
                        <?php $attributes = array('name' => 'schedule_timetable', 'id' => 'schedule_timetable','class'=>'form-inline'); echo form_open_multipart('', $attributes);?>
                            <input type="hidden" value="<?php if($this->uri->segment(2)){ echo $this->uri->segment(2);} ?>" name="serial" id="serial">
                               <div class="form-group">
                                  ...
                                  <select   ng-options="item.name for item in classlist track by item.id"  name="select_class" id="select_class"   ng-model="select_class"></select>
                                      </div>
                                      <div class="form-group">
                                      ...
                                      <div class="form-group">
                                        <label for="inputSemester">Semester<span class="required"></span></label>
                                         <select  ng-options="item.name for item in semesterlist track by item.id"  name="inputSemester" id="inputSemester"  ng-model="inputSemester" my-repeat-directive>
                                                                                </select>
                                      </div>
                                      <div class="form-group">
                                        <label for="file">Import<span class="required"></span></label>
                                         <input type="file" name="file" id="file" class="form-control">
                                      </div>
                                      <div class="form-group">
                                       <button type="button" class="nowsave" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Uploading...">Upload</button>
                                    </div>
                ...
                <div class="row">
                  <div class="col-sm-12">
                      ...
                      <p>
                            <button  type="button" id="export-file"  class="export_button" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Exporting...">
                                    Export
                          </button>
                          <button name="save"  id="saveupdate" class="intext-btn sve" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Saving...">Save</button>

                    <button name="Delete"  id="DeleteLesson3"  class="del intext-btn sve" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Deleting...">Delete Default Plan</button>
                              </p>
                  </div>
                </div>
    </div>
     ...
     // loadSections() ;
      ...
      function getSubjectList()
      {
        try{
            ...
            httprequest('getsubjectlistbyclass',data).then(function(response){
...
$(document).on('click','#DeleteLesson',function(){
        ...
        httprequest('<?php echo $path_url; ?>getsemesterdata',({})).then(function(response){
          if(response.length > 0 && response != null)
          {
            ...
            for (var i = 0; i < response.length; i++) {
                if(response[i].active_semster == 'a')
                {
      ...
      function responseFail(response){
        return (response.data);
            ...
            var
             ...
             $scope.loading_data = 1;
              ...
              function get_data(classid,subjectid,inputSemester)
               {
                ...
                var userdata = [
                  ...
                  $.ajax({
                    ...
                    data: {classid:classid,subjectid:subjectid,semesterid:inputSemester},
                    success: function (res) {
                      $scope.defaultresponse = []
                ...
                var page  = $scope.page
                  ...
                  for (; row < count && row< $scope.defaultresponse.length; row++) {
                    ...
                    var temp = []
                     ...
                     temp.push($scope.defaultresponse[row].day)
               ...
               var container = $("#example1").handsontable({
                ...
                height: ($scope.defaultresponse.length < 10 ? 450 : 500 ) ,
                 columns: [
                  ...
                  {
                    type: 'dropdown',
                    source: ['Document', 'Video', 'Image', 'Text','Application']
                  },
                  {data: 5},
                 {renderer: "html"},
                  {data: 7},
            ...
            $this.button('loading');
              var jsonString = JSON.stringify(part);
                    ...
                    $.ajax({
                     url:'Savedata',
                      type: 'POST',
                      data: {data:jsonString,classid:classid,sectionid:sectionid,subjectid:subjectid,semesterid:semesterid},
                      success: function(res){
             ...
             $(document).on('click','#Addprqst',function(){
             var id = $("#example1").handsontable('getDataAtRow',1,1);
                  var classid=$("#select_class").val();
                  var subjectid=$("#select_subject").val();
                  var semesterid=$("#inputSemester").val();
                      $.ajax({
                      url:'Prerequisite',
                      type: 'POST',
                      data: {classid:classid,subjectid:subjectid,semesterid:semesterid},
                      success: function(res){
                     window.location.href = "Prerequisite";
        ...
        $(document).on('click','#uploadfileReady',function(){
            ...
            data.append("subjectname",$scope.inputSubject.title);
            data.append('id',id)
            message('File Uploading','show')
            $.ajax({

                url: '<?php echo $path_url;?>Principal_controller/uploadContent?files',
            ...
            $this.button('loading');
              $.ajax({
                     url:'exportdata',
                      type: 'POST',
                      dataType: 'json',
                      data: {classid:classid,sectionid:sectionid,subjectid:subjectid,semesterid:semesterid},
                      success: function(data){
