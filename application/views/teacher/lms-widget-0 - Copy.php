                                                                ...
                                                                <div class="panel panel-default">
                                                                    <div class="panel-heading">
                                                                        <h4 class="panel-title cheading">
                                                                            <a data-toggle="collapse" aria-expanded="false" class="detail-section {{sub.cssclass}} collapsed" ng-click="getSubjectProgressReport(sub.sbid,s.sid,p.semsterid,p.sessionid,p.classid)" data-parent="#data_attributes" href="#c{{sub.sbid}}{{s.sid}}">
                                                                            Course Progress</a>
                                                                        </h4>
                                                                    </div>
    ...
    app.controller('progress_ctrl', function($scope, $window, $http, $document, $timeout,$interval,$compile){
        ...
        function reloadresult()
        {
            sinterval = $interval(function(){
                ...
                getQuizDetail($scope.subjectid,$scope.classid,$scope.sectionid,$scope.semesterid,$scope.sessionid)
            },20000);
        }
        ...
        $scope.getSubjectProgressReport = function(subjid,sectionid,semsterid,sessionid,classid){
            ...
            reloadcontent()
             if (angular.isDefined(sinterval)) {
                    ...
                    if(response != null && response.length > 0)
                    {
                        ...
                        for (var i = 0; i <= response.length-1; i++) {
                            ...
                            for (var k = 0; k <= response[i].studentplan.length - 1; k++) {
                               if(response[i].studentplan[k].status == 'read')
                               {
                                    cont_str += '<td class="'+response[i].studentplan[k].status+'"><i class="fa fa-check" aria-hidden="true"></i></td>'; 
                               }
                               else{
                                  cont_str += '<td class="'+response[i].studentplan[k].status+'"></td>';
                               }
                            }
                            cont_str += '</tr>';
                        }
                ...
                httprequest(urlist.getevulationheader,({
                    ...
                    if(response != null && response.length > 0)
                    {
                        ...
                        for (var i = 0; i <= headerlenght; i++) {
                            ...
                            if($scope.midterm == i  && bt_end == false){
                                bt_end = true
